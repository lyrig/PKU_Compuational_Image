...
class Ray{
    public:
        ...
        double time;
        Ray() {}
        Ray(const Point3& p, const Vector3& d, double tm=1e5, double time=0.0) : pos(p), dir(d), tMax(tm), time(time) {}
...
struct Triangle{
    ...
    {
        Vector3 v0 = V4to3(vertices[0].pos);
        Vector3 v1 = V4to3(vertices[1].pos);
        Vector3 v2 = V4to3(vertices[2].pos);
        ...
        if(u >= -1e-3 && v >= -1e-3 && u + v <= 1 && abs(eps) < 0.001)
        {
            return true;
        }
...
struct Model{
    ...
    void InitCube(float scale = 0.5){
        // 初始化 正方体的模型，贴图信息
        vertices.push_back(Vector3(-scale, -scale, -scale));
        vertices.push_back(Vector3( scale, -scale, -scale));
        vertices.push_back(Vector3(-scale,  scale, -scale));
        vertices.push_back(Vector3( scale,  scale, -scale));
        vertices.push_back(Vector3(-scale, -scale,  scale));
    ...
    void InitWall(float scale=1.0){
        // 初始化 正方体的模型，贴图信息
        vertices.push_back(Vector3(-scale, -scale, -0.0));
        vertices.push_back(Vector3( scale, -scale, -0.0));

        vertices.push_back(Vector3(-scale,  scale, -0.0));
        vertices.push_back(Vector3( scale,  scale, -0.0));

        vertices.push_back(Vector3(-scale, -scale,  0.1));
        vertices.push_back(Vector3( scale, -scale,  0.1));
...
class Camera{
    ...
    public:
        ...
        {
            double n = zNear;
            double tanFofV = tan((FofV / 360.0) * PI);
            double y = 2 * n * tanFofV;
            return Vector3(0.0, y / 2.0, 0);
        }
        Vector3 GetXspace()
        {
            double n = zNear;
            double tanFofV = tan((FofV / 360.0) * PI);
            double x = ratio * 2 * n * tanFofV;
            return Vector3(x / 2.0, 0.0, 0);
        }
        void setCamera(const Vector4& _eyePos = Vector4(-1.0, 0.0, 0.0, 0.0),
        ...
        Vector3 GetLeftTopDir()
        {
            double n = zNear;
            double tanFofV = tan((FofV / 360.0) * PI);
            double y = 2 * n * tanFofV;
            double x = ratio * y;
            auto dir = Vector3(-x / 2.0, y / 2.0, n);
            dir.normalize();
            return dir;
        }
        Vector3 GetCenter()
        {
            ...
            double y = 2 * n * tanFofV;
            double x = ratio * y;
            auto dir = Vector3(0.0, 0.0, n);
            return dir;
        }
        Vector3 GetRightTopDir()
        {
            double n = zNear;
            double tanFofV = tan((FofV / 180.0) * PI);
            double y = 2 * n * tanFofV;
            double x = ratio * y;
            auto dir = Vector3(x / 2.0, y / 2.0, n);
            dir.normalize();
...
class Rendering{
    public:
        ...
        int renderType = 0; // 渲染方式，0代表标准渲染，1代表卡通化
        Color defaultColor;
        /*
        0: 只绘制三角面片框架
        1: 不使用 mipmap 进行渲染
        ...
        */
        ...
        vector<Triangle> allTriangle;  // 需要渲染的三角面片
                ...
                if (renderType == 1)
                {
                    ...
                    for(int x = 0; x < 2*_h; ++ x)
                    {
                        for(int y = 0; y < 2* _w; ++y)
                        {
                            ...
                            col.x = *(ptr + 2);
                            bool check = true;
                            if(col.x == defaultColor.x && col.y == defaultColor.y && col.z == defaultColor.z)
                            {
                                ...
                                for(int tx = x - delta; tx < x + delta; ++ tx)
                                {
                                    for(int ty = y - delta; ty < y + delta; ++ty)
                                    {
                                        ...
                                        // std::cout << sqrt((x - tx) * (x - tx) + (y - ty) * (y - ty)) << std::endl;
                                        if(sqrt((x - tx) * (x - tx) + (y - ty) * (y - ty)) < delta)
                                        {
                                            ...
                                            tcol.y = *(tptr + 1);
                                            tcol.x = *(tptr + 2);
                                            if(tcol.x != defaultColor.x || tcol.y != defaultColor.y || tcol.z != defaultColor.z)
                                            {
                    ...
                    for(int j = -_h; j < _h; ++j) // Height
                    {
                        ...
                        for(int s = 0; s < 1; ++s)
                        {
                            ...
                            if(check)
                            {
                                ...
                                // std::cout << "Hit Pos: "<<pHit<<std::endl;
                                // pHit = camera.transProject(pHit);
        ...
        // biaoji
        void GetColor(Triangle &tri, const Model &model)
        {
            for(int i = 0; i < 3; ++ i)
            {
                // tri.vertices[i].pos.ViewportTransform(2 * _w, 2 * _h);
                Vertex vertex = tri.vertices[i];
                double w = 1.0 / tri.vertices[i].rhw;
                ...
                tri.vertices[i].col.x = (model.texture)->_bit[u][v * 3 + 0] / 255.0;
                tri.vertices[i].col.y = (model.texture)->_bit[u][v * 3 + 1] / 255.0;
                tri.vertices[i].col.z = (model.texture)->_bit[u][v * 3 + 2] / 255.0;
                // std :: cout << tri.vertices[i].col.z  << std:: endl;
            }
        }
        ...
        Vector3 computeRay(int i, int j)
        {
            Vector3 dir = camera.GetCenter();
            // std :: cout << "DIDIDIDID : "<< dir << std::endl;
            dir = dir - camera.GetYspace() * j / float(_h) - camera.GetXspace() * i / float(_w);
            dir.normalize();
            return dir;
        }
        ...
        void drawScanline(int y, Vertex verL, Vertex verR, const Triangle& tri, const Model& model, const double Miplevel = 0.0){
            ...
            for (int x = max(0, xl); x < xr && x < 2 * _w; ++x){
                ...
                loc = camera.InvtransProject(loc); // 原始坐标系
                if(renderType == 0)
                    col = BlinnPhong(col, loc, normal, 0b111); 
                else if(renderType == 1)
                    col = Toon(col, loc, normal);
        ...
        Color InterpolateColor(const Vector3 &loc, const Triangle &tri); // 插值计算三角面片任何一点的颜色。

        Color BlinnPhong(Color col, const Vector4 &loc, const Vector4 &Norm, int type=0b111);
        Color Toon(Color col, const Vector4 &loc, const Vector4 &Norm);
