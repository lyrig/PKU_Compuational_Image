...
int main(){
    std::cout << "Judging problem3\n";
    srand(20240314);
    Rendering device(Width, Height, screen_fb);
    device.switchMode() = 2; // bilinear texture interpolation
    ...
    BlackWhite.LoadBlackWhiteSmall();
    BlackWhite.initMipmap();
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 1; j < 3; j++){
            ...
            if ((i + j + 1) & 1){
                ...
                Matrix4 scale;
                scale.SetScale(1.2, 1.2, 1.2);
                circ.WorldMat = translate * rotate * scale;
    ...
    std::cout << "Begin Draw1."<<endl;
    device.drawScene(Scene);
    std::cout << "Begin Draw2."<<endl;
    device.drawScreen();
    std::cout << "Begin Draw3."<<endl;
    SaveImage();

    std::cout << "Finish judging problem3\n";
